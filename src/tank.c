...
#define TANK_HULL_ROTATION_ANGLE 0.03
#define TANK_TRACK_COLOR DARKGRAY
#define TANK_TURRET_COLOR GREEN
#define TANK_TURRET_ROTATION_ANGLE 0.06
...
void tank_move(Tank *t, int dir) {
    t->hull_rec.x += dir * t->hull_dir.x;
    t->hull_rec.y += dir * t->hull_dir.y;
}

void tank_rotate(Tank *t, int dir) {
    t->hull_dir = Vector2Rotate(t->hull_dir, dir * TANK_HULL_ROTATION_ANGLE);
}

void tank_turret_rotate(Tank *t, int dir) {
    t->turret_dir = Vector2Rotate(t->turret_dir, dir * TANK_TURRET_ROTATION_ANGLE);
}
...
void tank_draw(Tank *t) {
    ...
    Vector2 turret_pos = {t->hull_rec.x, t->hull_rec.y};
    DrawCircleV(turret_pos, t->turret_radius, TANK_TURRET_COLOR);
    Vector2 barrel_pos = Vector2Add((Vector2){t->hull_rec.x, t->hull_rec.y},
            Vector2Scale(t->turret_dir,
                t->turret_radius + TANK_BARREL_LENGTH / 2 - TANK_TURRET_RADIUS * 0.05));
