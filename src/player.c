...
#define PLAYER_SPEED_INITIAL 2.0
...
void player_move(Player *p) {
    if (Vector2Length(p->dir_target) > 0) {
        ...
        float angle = PLAYER_TURN_SPEED;
        float d = Vector2DotProduct(normal, p->dir_target);
        if (d > 0) angle *= -1;
        float angle_to_target = Vector2Angle(p->dir, p->dir_target);
...
void player_shoot(Player *p, List *bs) {
    if (p->shot_delta >= PLAYER_SHOT_DELAY) {
        Vector2 bullet_start = Vector2Add(p->pos, Vector2Scale(p->dir, p->radius + BARREL_LENGTH - BULLET_RADIUS * 2 - (float)BARREL_LENGTH/20));
...
void player_draw(Player *p) {
    ...
    Vector2 barrel_pos = Vector2Add(p->pos, Vector2Scale(p->dir, p->radius + BARREL_LENGTH/2 - (float)BARREL_LENGTH/20));
...
Vector2 player_get_pos(Player *p) {
    return p->pos;
}
